...
unless $debugdebug then 
  $adminary = [ "ict@ray.co.jp" ] # yasu@ray.co.jp", "d-furuya@ray.co.jp","a-shigemune@ray.co.jp","ict@ray.co.jp"]
else 
  $adminary = [ "ict@ray.co.jp" ] # "mtest@ray.co.jp", "ken_root@ray.co.jp" ]
end 
...
$account = ARGV[0] 
if $account == 'ken@ray.co.jp' then 
  $account = 'ken@ss.ray.co.jp' 
end
...
# byebug 
  if $deb then 
    mail.to = 'ict@ray.co.jp' 
    mail.cc = "" 
  else
   #  $to.push($to_cl)  
    mail.to = 'ict@ray.co.jp'  # $to.uniq.join(",") 
