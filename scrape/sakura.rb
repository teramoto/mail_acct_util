...
require 'mail'
require 'nkf' 
require '/var/www/html/foundation-4/admin/scrape/humanreadable.rb' 
...
$adminaddr = [ "t-niijima@c.ray.co.jp", "mail-support@ray.co.jp"  ]
...
#  puts "#{num}:#{tr}" 
  puts "---------------------" if $deb
  if tr != nil && (tx = tr.search('span[@class="description"]')) then 
    # byebug
    td = tr.search('td')  
...
#      puts desc , user
      ...
      if $deb then 
        puts "#{desc},#{address},mail:#{mail}ftp:#{ftp}, used:#{usedt}/#{capat}, ratio = #{ratio} % " 
      end 
      address = user + '@' + dom
      if mail = "◯" && ratio > 80   then 
        comment = ""  
        warnuser(address, address , ratio , usedt, capat , comment) 
      end 
    end 
