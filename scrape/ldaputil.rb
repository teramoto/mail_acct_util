...
def getpass(email)
  ldap = getldap(email)
  puts "ldap=#{ldap}" if $deb
  if ((ldap == true) || (ldap == nil)) then 
    STDERR.puts("cannot get ldap server.") if $deb  
    return true 
  else
    bb = email.split('@') 
    if bb.size !=2 then
      STDERR.puts('invalid email.(no \'@\' ') 
      return true 
    end 
#    elsif bb[1] == 'ray.co.jp' then 
#      uid = bb[0]
#    else 
#      uid = email
#    end
     
    res = ldapvalue( 'mail', email , 'userPassword', ldap)
    STDERR.puts("Error on get pass. #{email},#{ldap}-- #{res}") if res == true  
    return res 
  end 
end
#
...
def kanayomi(pass)
  ...
  elsif  (pass.length) == 0 then 
    return "Error in ldaputil/kanayomi pass.length==0 " 
  end
  pass.each_byte do |c|
